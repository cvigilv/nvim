	...
	if ok then
		if entry ~= nil then
			...
			return vim.tbl_map(function(e)
				return string.format("#%06x", e)
			end, hl)
		end
	else
		vim.notify("[ERROR] Highlight group `" .. hl_group .. "` doesn't exist!", vim.log.levels.ERROR)
		return nil
	end
end
...
M.swap_colors = function(color_table)
	if vim.tbl_contains(vim.tbl_keys(color_table), "fg") and vim.tbl_contains(vim.tbl_keys(color_table), "bg") then
